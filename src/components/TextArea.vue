...
const emit = defineEmits(["update:modelValue"]);//update:modelValue：这是一个约定俗成的事件名称，当子组件需要更新父组件中的 v-model 绑定的值时，通常会触发这个事件，并传递新的值。

const {modelValue, placeholder = "", autoSize = {minRows: 2, maxRows: 5}} = props

const value = useModel(modelValue, props, "modelValue", emit, "update:modelValue") //这里传入的emit是调用父组件的update:modelValue，实现更新父组件的v-model绑定的值
